...
import { Search, ChevronDown, Users, Armchair, SlidersHorizontal } from 'lucide-react'
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div>
          <label className="block text-xs font-body text-warm-500 mb-1">
            <Armchair className="h-3 w-3 inline mr-1" />
            Cabin
          </label>
          <select
            value={cabin}
      ...
      <div>
        <button
          ...
          className="flex items-center gap-2 text-sm font-display font-medium text-warm-500 hover:text-warm-700 transition"
        >
        ...
        {showAdvanced && (
          <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2">
            {strategies.filter(s => !s.default).map(s => (
              <label
                key={s.id}
                className={`flex items-start gap-2 rounded-xl p-3 cursor-pointer transition border-2 ${
              ...
              >
                <input
                  ...
                  className="mt-0.5 rounded border-warm-300 text-coral-500 focus:ring-coral-400"
                />
                <div>
                  <div className="text-sm font-display font-medium text-warm-800">{s.name}</div>
                  <div className="text-xs font-body text-warm-400 mt-0.5">{s.description}</div>
                </div>
              </label>
            ))}
          </div>
